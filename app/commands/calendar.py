...
def create_event(params: dict):
    ...
    if not is_string_non_empty([summary, description, start_date, end_date]):
        return {'error': 'Values: summary, description, start_date, end_date should be non-empty strings'}
    if not start_date_in_future(start_date):
        return {'error': 'start_date cannot be in the past'}
